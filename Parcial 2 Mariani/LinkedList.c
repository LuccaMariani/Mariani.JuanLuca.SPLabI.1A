...
LinkedList* ll_map(LinkedList* pLista, int(*pFunc)(void*))
{
    int retorno=-1;
    void* pElement= NULL;

    if(pLista != NULL && pFunc != NULL)
    {
        for(int i=0; i<ll_len(pLista); i++)
        {
            pElement= ll_get(pLista, i);
            pFunc(pElement);
        }
        retorno=1;
    }
...
LinkedList* ll_filter_parametro(LinkedList* this, int (*pFunc)(void*,char*), char* parametro)
{
    ...
    if (this!=NULL)
    {
        ...
        for(int i=0; i<ll_len(this); i++)
        {
            ...
            if(pFunc(ll_get(this,i), parametro)!=0)
            {
                ll_add(auxLista,ll_get(this,i));
            }
        }
    }
